...
/*arcadius change : comment function fz_bound_page(ctx, page, &node->rect)*/
/*arcadius change : comment function pdf_array_push(ctx, annot_arr, ind_obj); on pdf-annot-edit.c*/
...
{
	LOGE("update_changed_rects: start 1");
	fz_context *ctx = glo->ctx;
...
{
	LOGE("deleteAnnotationInternal: start");
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	pdf_document *idoc = pdf_specifics(ctx, doc);
	...
	LOGE("deleteAnnotationInternal: start 1");
	fz_try(ctx)
	{
		...
		LOGE("deleteAnnotationInternal: start 1.5");
		if (annot)
		{
